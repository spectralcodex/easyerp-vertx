...
public class Slf4jRequestLogger implements RequestLogHandler {
  ...
  private void log(RoutingContext context, long timestamp,
    ...
    JsonObject message = new JsonObject().put("type" ,"request").put("service", "vertx-docker");
    switch (this.format) {
      case DEFAULT:
  ...
  public void handle(RoutingContext context) {
    //LOG.info("We are supposed to be loggginh!!!!!!!!!!");
    ...
    if (this.immediate) {
      ...
      context.addBodyEndHandler((handler) -> this.log(context, timestamp, remoteClient, version, method, uri));
    }
